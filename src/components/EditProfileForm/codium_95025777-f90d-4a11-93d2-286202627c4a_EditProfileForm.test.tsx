...
describe('addIconsToDB', () => {
    ...
    it('should return the added icon when fetch request is successful', async () => {
      // Mock the fetch function
      const mockFetch = jest.fn().mockResolvedValue({ json: () => Promise.resolve(icon) });
      global.fetch = mockFetch;

      // Call the function and get the result
      const result = await addIconsToDB(icon);

      // Check if fetch is called with the correct arguments
      expect(mockFetch).toHaveBeenCalledWith('/api/profileIcons', {
        method: 'POST',
        headers: {
          "ContentType": "application/json"
        },
      ...
      // Check if the result is equal to the added icon
