...
describe('addIconsToDB', () => {
    ...
    // Can add multiple icons to the database
    it('should add multiple icons to the database', async () => {
      // Mock the fetch function
      const mockFetch = jest.fn().mockResolvedValue({ json: () => Promise.resolve({}) });
      global.fetch = mockFetch;

      // Call the function multiple times with different icons
      await addIconsToDB(icon1);
      await addIconsToDB(icon2);
      await addIconsToDB(icon3);

      // Check if fetch is called with the correct arguments for each call
      expect(mockFetch).toHaveBeenNthCalledWith(1, '/api/profileIcons', {
        method: 'POST',
        headers: {
          "ContentType": "application/json"
        },
        body: JSON.stringify(icon1)
      });
      expect(mockFetch).toHaveBeenNthCalledWith(2, '/api/profileIcons', {
        method: 'POST',
        headers: {
          "ContentType": "application/json"
        },
        body: JSON.stringify(icon2)
      });
      expect(mockFetch).toHaveBeenNthCalledWith(3, '/api/profileIcons', {
        method: 'POST',
        headers: {
          "ContentType": "application/json"
        },
        body: JSON.stringify(icon3)
      });
    });
});
