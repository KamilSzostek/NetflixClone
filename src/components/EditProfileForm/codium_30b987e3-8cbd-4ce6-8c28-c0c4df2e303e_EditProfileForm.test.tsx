...
describe('addIconsToDB', () => {
    ...
    // Throws an error if the icon is not a valid image
    it('should throw an error when the icon is not a valid image', async () => {
      // Mock the fetch function to resolve with an error response
      const mockFetch = jest.fn().mockResolvedValue({ json: () => Promise.resolve({ error: 'Invalid image' }) });
      global.fetch = mockFetch;

      // Call the function and expect it to throw an error
      await expect(addIconsToDB(icon)).rejects.toThrow('Invalid image');
    });
});
